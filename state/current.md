...
截至 2026-05-04，若以 staging 等价镜像环境为功能验收口径，系统已具备以下准生产级能力：
...
- M4 runtime policy 文件型 provider 在 staging DaemonSet 中可用：ConfigMap `runtime_policy` 挂载为 `/etc/crawler/runtime/runtime_policy.json`，支持热加载、LKG、作用域 pause、deadline expired、retry exhausted 和 SIGTERM shutdown 指标。
- Prometheus 已能观察 sticky-pool、pacer、多出口请求结果、反馈信号、执行态读写和依赖健康。

production 仍需按 staging 同一流程复刻验证；Grafana / 告警、长期压测、Kafka outbox、DLQ 和 IaC 不属于当前已完成能力。运行时 effective policy、pause、`deadline_at` / `max_retries` 生效和严格优雅停机入口已在 M4 本地与 staging 完成。
...
| Politeness 策略 | 部分完成 | 已忽略 robots.txt，并保留并发、延迟、重试 fallback；005 已实现 ADR-0012 的自适应防封闭环，本地与 staging 验证通过。007 已以本地文件 / ConfigMap provider 实现 effective policy 热加载与 last-known-good，并在 staging 验证通过；后续控制平面可输出同形态策略。 |
| 分布式调度只读消费 | 完成 P2 目标节点验证；M4 严格停机入口已收口 | 003 已验证 Redis Streams consumer group 单 worker、多 worker、fetch failed、无效消息和 Kafka failure / PEL reclaim；不引入 scrapy-redis 默认 scheduler / dupefilter。只读边界脚本已覆盖 key diff 与目标 stream `XLEN` 前后不变。003 早期目标节点验证曾显示 SIGTERM 后 shutdown flag 触发较晚；007 已补更早 signal handler，并在 staging 观察到 `fetch_queue_shutdown_signal_received` 与 `fetch_queue_shutdown_loop_exit`。 |
...
| Prometheus 指标 | 部分完成 | 已有请求、耗时、IP、黑名单、存储、Kafka producer 指标；005 已补 sticky-pool、pacer、delayed buffer、feedback、cooldown、slowdown、Redis 执行态指标；007 已补 policy load、policy version、LKG、pause、deadline、retry terminal、shutdown 指标并在 staging 观察到对应计数；集群级队列、lag、资源面板未完成。 |
...
- D-DEBT-3：005 已补齐 env / ConfigMap 参数化的自适应防封闭环，并通过 staging 验证；007 已先用本地文件 / ConfigMap provider 实现 effective policy 热加载、last-known-good 和作用域 pause，并通过 staging 验证，后续再接控制平面运行时下发。
...
- D-DEBT-6：T015c 曾只满足 PEL 不清空与可恢复底线；007 已补更早 SIGTERM / SIGINT shutdown flag 入口，并在 staging 验证 shutdown 信号日志与退出总结。production 复刻前仍建议补带 in-flight / delayed buffer 的专项停机场景。
...
- 对象存储上传结果。
- Kafka producer 发布结果。
- 007 已补 policy load、policy version、last-known-good、pause、deadline expired、max retries terminal、shutdown drain 等运行时控制指标；staging 已观察到对应计数，仍需生产看板化。
