...
- 尚未交付第五类消费端事实投影。
- 运行时 effective policy 热加载、last-known-good、作用域 pause、`deadline_at` / `max_retries` 生效和严格优雅停机入口已在 007 本地实现并通过测试；staging / production 环境验证未执行。
...
production 仍需按 staging 同一流程复刻验证；Grafana / 告警、长期压测、Kafka outbox、DLQ 和 IaC 不属于当前已完成能力。运行时 effective policy、pause、`deadline_at` / `max_retries` 生效和严格优雅停机入口已在 M4 本地完成。
...
| Politeness 策略 | 部分完成 | 已忽略 robots.txt，并保留并发、延迟、重试 fallback；005 已实现 ADR-0012 的自适应防封闭环，本地与 staging 验证通过。007 已以本地文件 / ConfigMap provider 实现 effective policy 热加载与 last-known-good，后续控制平面可输出同形态策略。 |
...
- D-DEBT-3：005 已补齐 env / ConfigMap 参数化的自适应防封闭环，并通过 staging 验证；007 已先用本地文件 / ConfigMap provider 实现 effective policy 热加载、last-known-good 和作用域 pause，后续再接控制平面运行时下发。
...
- 对象存储上传结果。
- Kafka producer 发布结果。
- 007 已补 policy load、policy version、last-known-good、pause、deadline expired、max retries terminal、shutdown drain 等运行时控制指标；仍需生产看板化。
