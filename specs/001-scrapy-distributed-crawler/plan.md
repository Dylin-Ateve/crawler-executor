...
- Gate 1：worker 启动并发现至少两个可用本地 IP。
- Gate 2：外部 echo endpoint 观测到多个预期 EIP。
- Gate 3：Host/IP 黑名单可以通过 Redis TTL 进入并退出冷却。
- Gate 4：metrics endpoint 可以报告抓取计数和 IP 健康。
- Gate 5：24 小时运行达成目标，或输出明确瓶颈报告。
- Gate 6：错误率完成记录即可，不设置硬性通过阈值。
