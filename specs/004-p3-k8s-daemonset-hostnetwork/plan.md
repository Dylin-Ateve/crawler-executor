...
| 决策门禁 | `state/decisions/` | 通过 | 遵守 ADR-0003、ADR-0004、ADR-0006、ADR-0008、ADR-0009、ADR-0010、ADR-0013；ADR-0011 仅保留历史背景。 |
...
M3 采用 ADR-0009 的 PEL 可恢复姿态，并按 ADR-0013 使用 `RollingUpdate maxUnavailable=1` 作为当前默认 rollout 策略。
...
- `safety_margin_ms=90000`
- `FETCH_QUEUE_CLAIM_MIN_IDLE_MS=600000`
- `updateStrategy=RollingUpdate`
- `rollingUpdate.maxUnavailable=1`
- 不承诺 `FETCH_QUEUE_SHUTDOWN_DRAIN_SECONDS` 强制终止 Scrapy in-flight
- 该默认值服务低频手动滚动，并覆盖 005 本地 delayed buffer、下载重试与 Kafka delivery timeout 的常见窗口，避免仍在处理中的 PEL 被其它 worker 过早 `XAUTOCLAIM`；超过该窗口的异常长尾仍依赖 `attempt_id` 幂等接受少量重复抓取
