...
**暂停原因**：生产部署前发现功能性遗漏，需先通过 005 明确并补齐。004 暂停在资源准备和现场记录阶段。
...
**恢复入口**：005 本地验证脚本已通过；恢复 004 前仍需在目标节点完成 005 smoke，并从“Step 2：DaemonSet dry-run”前的 ConfigMap 审核继续，随后再执行 DaemonSet 审计、IP 池、消费、debug、pause 与 PEL 验证。
...
| `deploy/environments/staging.env` | 复刻 production 功能口径：同 namespace / workload / node label key / 005 策略；仅保留 staging 物理差异，如 `CRAWL_INTERFACE=ens3`、`M3_IP_POOL_EXPECTED_RANGE=50-60`、staging 存储与 Kafka / Redis 端点。 |
