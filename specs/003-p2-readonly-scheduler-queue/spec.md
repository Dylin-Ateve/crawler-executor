...
- **相关 ADR**：ADR-0002、ADR-0003、ADR-0004、ADR-0005、ADR-0006、ADR-0007、ADR-0008、ADR-0010。
...
- worker 在 `XREADGROUP` 阻塞窗口内收到 SIGTERM / SIGINT。
- worker 在 in-flight 请求未完成时被 SIGTERM / SIGINT 触发优雅停机。
- 停机 drain 时限到达后仍有 in-flight 请求未完成。
- 停机期间 Kafka 仍不可达，已开始的请求无法发布 `crawl_attempt`。
...
- **FR-017**：超过最大投递次数后，必须发布终态失败 `crawl_attempt`，然后 `XACK`。本约束**仅适用于 fetch 层失败**（DNS、TCP、超时、可重试 HTTP 状态码及其它由 spider errback / `_should_retry_request` 判定的可重试态）；Kafka 发布失败不进入本投递次数终态语义，按 ADR-0008 处理（不 ack、可被 `XAUTOCLAIM` 接管、不递增 attempt 级 retry 计数、不强制发布终态 attempt）。
...
- **FR-022**：worker 必须在收到 SIGTERM 或 SIGINT 时进入优雅停机路径，按 ADR-0009 不变量执行：
  - 停止从 Redis Streams 读取新消息（停止 `XREADGROUP`）。
  - 停止接管其它 consumer 的 PEL（停止 `XAUTOCLAIM`）。
  - 允许 in-flight 请求完成；缺省 drain 时限为 25 秒，可通过 `FETCH_QUEUE_SHUTDOWN_DRAIN_SECONDS` 调整。
  - 不主动清空本进程 PEL；未完成消息留给后续 `XAUTOCLAIM` 接管。
  - 已达到 `max_deliveries` 上限的 fetch 终态失败仍按 FR-017 / ADR-0006 发布终态 `crawl_attempt` 后 `XACK`。
  - Kafka 不可达分支按 FR-016 / ADR-0008 处理：不 `XACK`、留 PEL、不递增投递计数。
  - 不在退出前主动重新 `XAUTOCLAIM` 以 "赶在退出前发完"。
  - 收到停机信号时与退出前必须各产生一条结构化日志和 `fetch_queue_event="shutdown"` 计数。
  - SIGHUP / SIGQUIT 不触发本路径，沿用进程默认行为。
...
- **SC-008**：worker 收到 SIGTERM / SIGINT 后，停止 `XREADGROUP` 与 `XAUTOCLAIM`；in-flight 请求在 drain 时限内完成的发布 `crawl_attempt` 后 `XACK`，未完成的留 PEL；进程在 drain 时限到达前退出，且本进程不主动 ack 任何 in-flight 之外的 PEL 消息。

## 目标节点验证记录

- 2026-04-30：`run-p2-graceful-shutdown-validation.sh` Phase 1 在目标节点失败：SIGTERM 后 worker 退出耗时 68 秒，超过脚本设置的 `FETCH_QUEUE_SHUTDOWN_DRAIN_SECONDS=8` + 5 秒容差；PEL 保持 `count=1`、`xlen=1`，满足不清空 PEL 的底线。
- 2026-04-30：日志显示 `fetch_queue_shutdown_signal_received` 在 `spider_closed` 阶段才记录，而非 SIGTERM 到达时记录；在此之前 Scrapy 已进入 `Closing spider (shutdown)`，但 consumer 尚未设置 shutdown flag。
- 2026-04-30：同一 `stream_message_id` 出现两次 `fetch_queue_response_observed`，且 PEL `max_times_delivered=2`，说明退出期间仍可能发生 `XAUTOCLAIM` / 重复处理。
- 2026-04-30：团队确认当前 K8s 场景为低频手动滚动、任务幂等、允许少量重复抓取。因此当前实现可作为过渡策略接受，但不视为严格满足 SC-008；后续严格收口仍需修正更早停机入口与 drain deadline 语义。
...
- 2026-04-29：确认 Kafka 发布失败不进入 FR-017 最大投递次数终态语义（详见 ADR-0008）。fetch 层失败按 FR-016 / FR-017 走"可重试 → 达上限发终态"路径；Kafka 失败保持"不 ack + PEL 留存 + 可被 reclaim"，由运维侧 Kafka 健康度告警与紧急停抓兜底，本仓库 003 阶段不引入本地 outbox 或丢弃 + ack 退化路径。
- 2026-04-30：确认优雅停机语义沉淀为 ADR-0009 与 FR-022。SIGTERM / SIGINT 同语义；SIGHUP / SIGQUIT 不纳入；停机期间禁止 `XAUTOCLAIM`；drain 时限缺省 25 秒；不接管 Scrapy 自身关停，采用共享停机标志 + spider 消费循环检查；阻塞中的 `XREADGROUP` 允许至多一个 `block_ms` 周期自然返回；停机期间失败分支沿用 ADR-0006 / ADR-0008。
